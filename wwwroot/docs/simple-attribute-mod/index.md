...
Click on the `.tkproj` file in your Project folder and fill in the fields with the relevant information on your mod. For a detailed explanation on the purpose of each field, please see the [TKMM Wiki](https://tkmm.org/docs/creating-mods/#basic-fields). Once you've configured the `.tkproj` file, right click the Project Folder in the Your Projects tab and choose `Package as TKCL`. Now you can share it however you'd like. Remember, in order to upload a `.tkcl` file on GameBanana, you have to put it inside of an archive first (`.zip`, `.7z`, `.rar`, etc).